...
- Note: RQ `Queue.enqueue()` có keyword arg reserved `job_id` (để set RQ job id) → DB job id được pass vào job function bằng positional arg, và RQ job id được set trùng DB job id để dùng chung.
...
- `GET /jobs/{job_id}`
  ...
  - nếu job có output (ví dụ cleanup), field `result` sẽ chứa JSON report
- `GET /jobs?limit=50` (tenant-scoped)
- `POST /jobs/{job_id}/retry?confirm=true` (admin key)
  - chỉ retry khi job `FAILED`/`DONE`
  - chặn job_type chưa được đánh dấu idempotent (có thể override bằng `force=true`)
...
### 6.12 Admin
- `POST /admin/cleanup?confirm=true&dry_run=true`
  - enqueue job `cleanup_storage` (tenant-scoped)
  - job `error` field sẽ chứa JSON report (dry-run mặc định)
- `GET /admin/queues` (admin key, yêu cầu Redis)
- `GET /admin/workers` (admin key, yêu cầu Redis)
