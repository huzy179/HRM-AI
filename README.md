...
2. **Company Policy Chatbot (Trợ lý Hội nhập):**
   ...
   - Chatbot giải đáp chính xác, tự nhiên mọi câu hỏi của nhân viên mới trích xuất từ dữ liệu nội bộ mà không bịa đặt thông tin (giảm hallucination).
...
*   **Mô hình Ngôn ngữ Lớn (LLMs & AI):**
    *   **Ollama:** Chạy Local LLM hoàn toàn offline (Windows/Mac/Linux).
    *   **LLM Core:** Llama-3 (8B) cho Chatbot sinh văn bản.
    *   **Embeddings:** `nomic-embed-text` cho việc tạo vector ngữ nghĩa.
*   **Khung RAG (RAG Framework):** LangChain (tích hợp Ollama embeddings/chat + retrieval).
*   **Cơ sở dữ liệu Vector (Vector DB):** ChromaDB (Local, lưu vector CV/JD/policy).
*   **Xử lý Tài liệu & NLP:** `PyMuPDF` (đọc PDF), `spaCy` (optional).
